...
a1.fadeout(32, fvol=.7)

a4 >> gone([0], dur=4, dull=0, body=0, arp=0, pitch=0).span(srot(48)).fadein(32)
a4 >> gone([0], dur=4, dull=0, body=0, arp=0, pitch=0).span(1).fadein(32)
...
s1 >> space([0], dur=[.5,.25,.25], oct=5, amp=P[.8,.7,.8,1.1]*1.5, sus=s1.dur+0.2, pan=1)
...
s2 >> blip([0], dur=P[.5,.25,.25]*2/3, oct=[5,8,6], amp=P[.8,.7,.8,1.1]*2, pan=1)
...
k1 >> play("X{.xX+}(X.)", dur=PDur(3,8), output=12).pause(8,32)
...
a2 >> fordrip(
    ...
    psy = 0,
    ambiance = 0,
).span(linvar(P[0, 1, .4, .8, 0, .6] * 6, PRand(2, 24), start=Clock.mod(4))).fadein(24)
...
m1 >> marimba("a", oct=P[3,4,5], amp=linvar([.1,.9],16, bus2=.5), dur=.125).humz().span(srot(64)).fadein()

m1 >> marimba("a[bb]fff...fff", oct=P[3,4,5], amp=linvar([.1,.9],16, bus2=.5), dur=.125).humz().span(srot(64))#.fadein()
...
b1 >> blip(chords, dur=cascara, sus=linvar([.3,3],16), oct=5, pan=1).pause(4,16,8)  #.mpan(mrot(24))
...
h2 >> play(".--(-*-[**])", sdb=1, sample=0, hpf=2000, dur=1/4, leg=10, pan=P[-1,0,1].stutter(3)).pause(2,7).mpan(mrot(24))

e3 >> play("*", dur=.25, sample=P[0,1,2].stutter(4), rate=1, amp=2).pause(4,16,12).mpan(mrot(32))
...
k4 >> play(".VVV", lpf=300, sample=7, amp=1, sdb=1, dur=1/4, room2=10, output=12)
...
d3 >> play("<.c.c.c.cc><...(...*)><..(i).>", dur=1/4, rate=var([1,1.3]), sample=var([1,2], 8), amp=3).mpan(mrot(32))
