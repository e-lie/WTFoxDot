...
a4 >> gone([0], dur=4, dull=0, body=0, arp=0, pitch=0).span(.5).fadein(32)
...
b1 >> blip([0,1,2,4,3], dur=[1,2], amp=2, sus=3)
...
k1 >> play(
    ...
    sample=3,
    pan=0,
).pause(8,16,4).every(.5,"stutter",2)
...
k1 >> play("X{.xX+}(X.)", dur=PDur(3,8), output=12).pause(24,32)
...
m1 >> marimba("abfff...ffbafff", oct=([2,5],[3,4,5],7), amp=linvar([.5,.9],16), dur=Pvar([.125, cascara/2], [8,4])).humz().span(.5)#.fadein()

d1 >> play("<(V----)..>", dur=.125)

m1 >> marimba("ab[d][cc]a.........b[aaaa]([c]a[cccc]d)[ff]", oct=3, amp=linvar([.1,.9],16, bus2=.5, vol=1.2, dur=.5), dur=.125).humz().span(srot(64))

m3 >> vibra("ab[d][cc]a.b[aaaa]([cccc]d)[ff]", oct=[2,3,5], amp=linvar([.5,.9],7), dur=cascara, bus4=.5).humz().span()
