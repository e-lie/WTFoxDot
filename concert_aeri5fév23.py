...
a1 >> apad([0,4,-2], dur=PRand(2,8), sus=PRand(4,12), vol=1.1, attack=.4, space=0, detail=0, thick_thin=0).span(.5).fadein(32)
...
a4 >> gone([0], dur=4, dull=0, body=0, arp=0, pitch=0).fadein(16, i)
...
a4.arp = 0# linvar([0,1,.4,.8,0,.6],PRand(2,24), start=Clock.mod(4))
a4.arp = 0
a4.gone_arp = linvar([0,1,.4,.8,0,.6],PRand(2,24), start=Clock.mod(4))
a4.oct=3
a4.span(srot(32))

a1.vol = linvar([1,0], 16, start=Clock.mod(4))

a4.vol = linvar([1,0], 16, start=Clock.mod(4))

a1.stop()

s1 >> space([0], dur=[.5,.25,.25], oct=5, amp=[.8,.7,.8,1.1], sus=s1.dur+0.2).mpan(0)
...
d7 >> bbass(chords, dur=PSum(3,2), oct=(2,3), amp=1, output=12) + [0,0,2] + [0,0,0,0,-1]
...
e1 >> play("*", sample=2, dur=1/4, amp=PRand(2)[:16]).mpan(mrot(24))
